          ...
          <ul class="navbar-nav">
              ...
              <div class="dropdown-menu row" aria-labelledby="topnav-dashboard" v-if="hasItems(item)">
                <template v-for="(subitem) of item.subItems">
                  <router-link :key="subitem.id" class="col dropdown-item side-nav-link-ref" v-if="!hasItems(subitem)"
                    :to="subitem.link">abcd</router-link>
                  <div class="dropdown" v-if="hasItems(subitem)" :key="subitem.id">
                    ...
                    <div class="dropdown-menu">
                      <router-link v-for="(subSubitem, index) of subitem.subItems" :key="index" :to="subSubitem.link"
                        class="dropdown-item side-nav-link-ref">abcd</router-link>
                    </div>
                  </div>
                </template>
              </div>
            ...
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle arrow-none" id="topnav-layout" role="button" data-toggle="dropdown"
                ...
                <i class="ri-layout-3-line mr-2"></i>abcd
                <div class="arrow-down"></div>
              </a>
              <div class="dropdown-menu dropdown-menu-right" aria-labelledby="topnav-layout">
                <a @click="change_layout('vertical')" class="dropdown-item">abcd</a>
                <a @click="topbarLight()" class="dropdown-item">abcd</a>
                <a @click="boxedWidth()" class="dropdown-item">abcd</a>
              </div>
            </li>
          </ul>
        </div>
